...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The Pub Game Store completes Stripe Express onboarding in test mode
- [x] #2 sellers.stripe_connect_account_id column is populated with the real Connect account id for the anchor seller
- [x] #3 apps/api/src/lib/stripe.ts and routes/checkout.ts confirmed to read/use this id correctly for the direct charge
- [x] #4 Verified the charge still uses { stripeAccount: connectedAccountId } + application_fee_amount (no custody of funds)
<!-- AC:END -->
