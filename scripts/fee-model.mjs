...
const FIXED = 500, SALARY = 40000

// Distribuciones de tamaño de orden (MXN). Singles media ~$436 (AOV doc: $400). Sellado media ~$2,560.
const SINGLES = [[80,.15],[150,.20],[250,.20],[400,.20],[700,.15],[1200,.07],[2500,.03]]
const SEALED  = [[800,.20],[1500,.30],[2500,.30],[6000,.20]]
...
const E = (d,f,cfg,mf,a) => d.reduce((o,[X,w])=>{const r=order(X,f,cfg,mf,a);return{rev:o.rev+w*r.rev,cost:o.cost+w*r.cost,net:o.net+w*r.net}},{rev:0,cost:0,net:0})

function month(gmv, mix, fS, fP, cfg, sellers, poutsPerSeller, mf=0, a=BASE){
  const gS=gmv*mix, gP=gmv*(1-mix)
  const nS=gS/mean(SINGLES), nP=gP/mean(SEALED)
  const eS=E(SINGLES,fS,cfg,mf,a), eP=E(SEALED,fP,cfg,mf,a)
...
console.log('fee | A: plat | B: plat | seller bruto A | seller efect A* | seller bruto B | seller efect B*')
for (const f of [.08,.09,.10,.11,.12,.13,.14]){
  const A=E(SINGLES,f,'A',0,BASE), B=E(SINGLES,f,'B',0,BASE), m=mean(SINGLES)
  const sBA=1-f, sEA=1-f/IVA
  const sBB=1-f-(S.proc*IVA + S.fix*IVA/400), sEB=1-f/IVA-S.proc-S.fix/400
  console.log(`${pct(f)} | ${pct(A.net/m)} | ${pct(B.net/m)} | ${pct(sBA)} | ${pct(sEA)} | ${pct(sBB)} | ${pct(sEB)}`)
}
console.log('* efect = seller con RFC que acredita IVA (fee y Stripe ex-IVA). Bruto B a orden de $400.')

// ── T2: barrido SELLADO ────────────────────────────────────────────────────────
console.log('\n== T2 SELLADO: take neto plataforma (%GMV sellado) ==')
console.log('fee | A: plat | B: plat | seller bruto B @$2500')
for (const f of [.04,.05,.06,.07,.08]){
  const A=E(SEALED,f,'A',0,BASE), B=E(SEALED,f,'B',0,BASE), m=mean(SEALED)
  console.log(`${pct(f)} | ${pct(A.net/m)} | ${pct(B.net/m)} | ${pct(1-f-(S.proc*IVA+S.fix*IVA/2500))}`)
}

// ── T3: orden mínima viable (contribución marginal ≥ 0, sin costos fijos) ─────
console.log('\n== T3: orden mínima viable por fee/config (solo costo marginal) ==')
for (const [f,cfg] of [[.09,'A'],[.10,'A'],[.13,'A'],[.09,'B'],[.10,'B']]){
  let X=10; while(X<2000 && order(X,f,cfg).net<0) X+=5
  console.log(`fee ${pct(f)} cfg ${cfg}: ~${mxn(X)}`)
}

// ── T4: bundles de política × escenarios ──────────────────────────────────────
const SC = [ ['Conservador', 80000, 2], ['Base', 332000, 5], ['Optimista', 1200000, 12] ]
const BUNDLES = [
  ['P0 hoy: A, 10% flat, sin min',        'A', .10, .10, 0 ],
  ['P1 docs: A, 9/5, min $15',            'A', .09, .05, 15],
  ['P2 A repriced: A, 13/7, min $25',     'A', .13, .07, 25],
  ['P3 B docs: B, 9/5, min $10',          'B', .09, .05, 10],
  ['P4 B rec: B, 10/6, min $15',          'B', .10, .06, 15],
]
...
for (const [name,cfg,fS,fP,mf] of BUNDLES){
  const row = SC.map(([,g,sel])=>{const m=month(g,.6,fS,fP,cfg,sel,4.33,mf); return `${mxn(m.net)} (${m.pct.toFixed(1)}%)`})
  // GMV*: sueldo; sellers escalan ~1 por cada $70k GMV (min 2)
  let g=100000, net=0
  while(g<6e6){ net=month(g,.6,fS,fP,cfg,Math.max(2,Math.round(g/70000)),4.33,mf).net; if(net>=SALARY) break; g+=10000 }
...
const VAR = [
  ['refund 0%',   {...BASE, refund:0}], ['refund 8%', {...BASE, refund:.08}],
  ['intl 0%',     {...BASE, intlShare:0}], ['intl 15%', {...BASE, intlShare:.15}],
  ['disputas 1%', {...BASE, disputeRate:.01}],
]
for (const [label,a] of VAR){
  const p2=month(332000,.6,.13,.07,'A',5,4.33,25,a), p4=month(332000,.6,.10,.06,'B',5,4.33,15,a)
  console.log(`${label}: P2(A 13/7) ${mxn(p2.net)} | P4(B 10/6) ${mxn(p4.net)}`)
}
// mix y AOV
for (const mix of [.8,.4]){
  const p2=month(332000,mix,.13,.07,'A',5,4.33,25), p4=month(332000,mix,.10,.06,'B',5,4.33,15)
  console.log(`mix singles ${mix*100}%: P2 ${mxn(p2.net)} | P4 ${mxn(p4.net)}`)
}
const SMALL = SINGLES.map(([x,w])=>[x*.6,w]) // AOV singles ~$262
{
  const eA=E(SMALL,.13,'A',25,BASE), eB=E(SMALL,.10,'B',15,BASE), m=mean(SMALL)
...
for (const [f,cfg] of [[.07,'A'],[.06,'B']]){
  const a={...BASE}; const o=order(2500,f,cfg,0,a)
  const msi = 0.05*2500
  const platNet = cfg==='A' ? o.net-msi : o.net
  const sellerHit = cfg==='B' ? msi : 0
  console.log(`cfg ${cfg} fee ${pct(f)}: neto plataforma ${mxn(platNet)} | costo extra seller ${mxn(sellerHit)}`)
}
